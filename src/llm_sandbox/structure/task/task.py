...
class Task:
    ...
    def get_task_arrays_for_llm_prompt(self, which_arrays: list | None = None) -> str:

        which_arrays = which_arrays if which_arrays is not None else list(range(len(self.input_canvases)))
        prompt = f'The images are:\n'
        for i, (ic, oc) in enumerate(zip(self.input_canvases, self.output_canvases)):
            if i in which_arrays:
