...
from llm_sandbox.structure.utils import union2d
from llm_sandbox.structure import constants as const
from visualization import visualize_data as vis
from llm_sandbox.structure.object.primitives import Primitive, Random, Dot, Predefined
from llm_sandbox.structure.geometry.basic_geometry import Point, Dimension2D, Surround, Surround_Percentage, Colour
    ...
    def show(self, full_canvas=False, fig_to_add: None | plt.Figure = None, nrows: int = 0, ncoloumns: int = 0,
        ...
        if full_canvas:
            ...
            if fig_to_add is None:
                fig, _ = vis.plot_pixels(self.full_canvas, extent=extent, thin_lines=thin_lines)
            else:
                ax = fig_to_add.add_subplot(nrows, ncoloumns, index)
                _ = vis.plot_pixels(self.full_canvas, extent=extent, axes=ax, thin_lines=thin_lines)
        else:
            xmin = - 0.5
            ...
            extent = [xmin, xmax, ymin, ymax]
            if fig_to_add is None:
                fig, _ = vis.plot_pixels(self.actual_pixels, extent=extent, thin_lines=thin_lines)
            else:
                ax = fig_to_add.add_subplot(nrows, ncoloumns, index)
                _ = vis.plot_pixels(self.actual_pixels, extent=extent, axes=ax, thin_lines=thin_lines)
