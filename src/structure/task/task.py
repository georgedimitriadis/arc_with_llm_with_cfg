...
class Task:
    ...
    def generate_token_for_llm(self):
        prompt = (f'What follows is a series of arrays each representing an image with low number of pixels. Each image\n'
                  f'can range from 3x3 to 32x32 pixels. Each pixel can be one of ten possible colours.\n'
                  f'Each pixel matters. There are {len(self.input_canvases)} training input - output pairs and {len(self.test_input_canvases)}\n'
                  f'testing inputs. There are two images for each training pair denoted as Train Input N and Train Output N\n'
                  f'(where N is the number of the training pair). There is one image per test input denoted Test N.\n'
                  ...
                  f'You have also been given a grammar to follow. After a little bit of thinking you will output the\n'
                  f'"\n<<<BEGIN>>>\n" \n'
                  f'token and then proceed to write the program that solves this logic using the\n'
                  f'functions provided by the grammar.\n\n'
        ...
        for i, ic in enumerate(self.test_input_canvases):
            icp = ic.actual_pixels
            prompt += f'Test Input {i}\n{self.array_to_text(icp)}\n'
